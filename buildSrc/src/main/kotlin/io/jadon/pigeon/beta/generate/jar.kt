...
object JarManager {
    ...
    val CLIENT_URL = URL("http://s3.amazonaws.com/Minecraft.Download/versions/b1.7.3/b1.7.3.jar")
    // TODO: Pretty sure this is illegal?
    val SERVER_URL = URL("https://jadon.io/files/minecraft_server.jar")

    fun downloadVanillaClient(destinationFile: String) {
        val path = Paths.get(destinationFile)
        path.parent.toFile().mkdirs()
        if (!path.toFile().isFile) {
            CLIENT_URL.openStream().use { Files.copy(it, path) }
        }
    }

    fun downloadVanillaServer(destinationFile: String) {
        val path = Paths.get(destinationFile)
        path.parent.toFile().mkdirs()
        if (!path.toFile().isFile) {
            SERVER_URL.openStream().use { Files.copy(it, path) }
        }
    }
...
//            } else true
        ...
        mergedFiles.addAll(remappedServerJar.entries().toList().filter {
            (it.name.endsWith(".class") && serverOnlyClasses.contains(it.name.removeSuffix(".class")))
